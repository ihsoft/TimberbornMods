...
public abstract class BeaverPopulationTrackerCondition : PopulationTrackerConditionBase {
  ...
  /// <seealso cref="RelativeToMaxLevel"/>
  ...
  /// <summary>
  /// Indicates that the threshold should be calculated based on the total dwelling space in the district.
  /// </summary>
  /// <seealso cref="Difference"/>
  public bool RelativeToMaxLevel { get; protected set; }
  ...
  protected override void OnBehaviorAssigned() {
    ...
    if (RelativeToCurrentLevel) {
      Threshold = Difference + DistrictPopulation.NumberOfAdults + DistrictPopulation.NumberOfChildren;
    } else if (RelativeToMaxLevel) {
      ...
      Threshold = Difference + dwellingStats.FreeBeds + dwellingStats.OccupiedBeds;
    } else {
  ...
  static readonly PropertyKey<bool> RelativeToMaxLevelKey = new("RelativeToMaxLevel");
  ...
  public override void SaveTo(IObjectSaver objectSaver) {
    ...
    objectSaver.Set(RelativeToMaxLevelKey, RelativeToMaxLevel);
    if (Threshold != -1) {
