...
public struct DropdownItem<T> where T : notnull {
  ...
  /// <summary>The icon to display in the dropdown or "null".</summary>
  public Sprite Icon;
  ...
  /// <summary>Implicit conversion from a tuple to the DropdownItem.</summary>
  public static implicit operator DropdownItem<T>((T value, string text, Sprite icon) tuple) {
    return new DropdownItem<T> { Value = tuple.value, Text = tuple.text, Icon = tuple.icon };
  }
}
