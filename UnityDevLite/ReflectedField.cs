...
sealed class ReflectedInternalField<TV> {
  readonly FieldInfo _fieldInfo;
  ...
  /// </param>
  /// <seealso cref="IsValid"/>
  public ReflectedInternalField(string fullTypeName, string fieldName, bool throwOnFailure = false) {
    ...
    var type = assembly.GetType(assemblyParts[2]);
    if (type == null) {
      throw new AggregateException($"Cannot get type '{assemblyParts[2]}' from assembly {assembly}");
    }
    _fieldInfo = type.GetField(fieldName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
    if (_fieldInfo == null) {
      if (throwOnFailure) {
