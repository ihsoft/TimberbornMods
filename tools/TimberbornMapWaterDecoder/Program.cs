...
static class Program {
  public static int Main(string[] args) => new WaterMapDecoderTool().Run(args);
}

sealed class WaterMapDecoderTool {
  public int Run(string[] args) {
    var waterDecoder = new WaterMapDecoder();
    ...
    if (args is ["--write-fixture", var fixtureMap, var fixturePath, var workshopId]) {
      ...
      WaterRegressionFixture.Write(
          fixturePath, workshopId, waterDecoder.Decode(fixtureWorld.RootElement, fixtureWidth, fixtureHeight));
    ...
    var water = waterDecoder.Decode(world.RootElement, width, height);
    var features = waterDiagnostics.Analyze(water);
    var classification = waterClassifier.Classify(water, features);
    ...
    File.WriteAllText(summaryPath, JsonSerializer.Serialize(new {
      ...
      shallow_flow_coherence_quantiles = GetQuantiles(
          SelectValues(water.SurfaceFlowCoherences, features.ShallowWaterMask)),
  ...
  static void WriteFeatureBitmap(
      string path, DecodedWaterMap map, WaterFeatureAnalysis features) {
